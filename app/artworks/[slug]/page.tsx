...
import { artworks, getArtwork, getOrderUrl, CHANNEL_URL } from "@/lib/artworks";
...
export async function generateMetadata({
  ...
  return artwork
    ? {
        title: `${artwork.title} — Luna Swan`,
        description: artwork.description,
      }
    : { title: "Artwork not found — Luna Swan" };
}
  ...
  return (
    <main className="min-h-screen bg-black text-white">
      ...
      <header className="border-b border-white/10 px-6 py-5">
        <nav className="mx-auto flex max-w-7xl items-center justify-between font-mono text-[10px] tracking-[0.2em]">
          <Link href="/" className="font-semibold tracking-[0.50em] ">
            LUNA SWAN®
          </Link>
          <Link
            href="/"
      ...
      <div className="mx-auto grid max-w-7xl gap-12 px-6 py-12 md:grid-cols-[1.15fr_0.85fr] md:items-start md:gap-20 md:py-20">
        {/* تصویر + Lightbox */}
        <ArtworkImage src={artwork.thumbnail} alt={artwork.title} />

        {/* اطلاعات اثر */}
        <article className="md:sticky md:top-10">
          <p className="font-mono text-[10px] tracking-[0.35em] text-white/45">
            LUNA SWAN / {artwork.category}
          </p>

          <h1 className="mt-5 font-mono text-3xl tracking-[0.12em] md:text-5xl">
            {artwork.title}
          </h1>

          <p
            dir="rtl"
            className="mt-7 max-w-lg font-sans text-base leading-8 text-white/65"
          >
            {artwork.description}
          </p>

          {/* مشخصات */}
          <div className="mt-10 grid grid-cols-2 border-y border-white/15 py-6 font-mono text-[10px] tracking-[0.12em]">
            <div className="space-y-5">
              <div>
                <p className="text-white/40">YEAR</p>
                <p className="mt-2 text-sm text-white">{artwork.year}</p>
              </div>
              <div>
            ...
            <div className="space-y-5">
              <div>
                <p className="text-white/40">TECHNIQUE</p>
                <p className="mt-2 font-sans text-sm text-white">
                  {artwork.technique}
                </p>
              </div>
              <div>
                <p className="text-white/40">EDITION</p>
                <p className="mt-2 font-sans text-sm text-white">
          ...
          <div className="mt-8 flex flex-wrap items-center justify-between gap-5">
            <div>
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                <p className="font-sans text-xl tracking-[0.08em]">
                  {artwork.price}
                </p>
              </div>
            </div>
            ...
            <a
              href={getOrderUrl(artwork)}
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-3  bg-white px-6 py-4 font-sans text-sm tracking-[0.18em] text-black transition-opacity hover:opacity-80"
            >
              سفارش اثر <ArrowUpRight className="size-5" />
            </a>
          </div>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mt-8 inline-flex items-center gap-2 font-sans text-sm tracking-[0.16em] text-white/45 transition-colors hover:text-white"
          >
            مشاهده آرشیو کامل در تلگرام <ArrowUpRight className="size-3" />
          </a>
        </article>
      </div>
    </main>
